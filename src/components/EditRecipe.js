...
const EditRecipe = ({ recipe, onSave, onCancel }) => {
  ...
  const addIngredient = () => {
    ...
    if (
      ...
      ingredients[ingredients.length - 1].ingredient_id
    ) {
      setIngredients([...ingredients, { ingredient: "", amount: "" }]);
    } else {
      // Optionally provide feedback to the user that the current row needs to be filled
      console.log(
        "Please fill the current ingredient row before adding a new one."
      );
    }
  };
  ...
  const handleIngredientSearchKeyDown = (e) => {
    ...
    switch (e.key) {
      ...
      case "Enter":
        ...
        if (
          highlightedIndex >= 0 &&
          highlightedIndex < ingredientSearchResults.length
        ) {
  ...
  return (
    <div className={`edit-recipe-container ${loading ? "is-saving" : ""}`}>
      {toast.visible && (
