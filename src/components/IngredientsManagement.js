...
const IngredientsManagement = ({ onAddIngredientClick }) => {
  ...
  useEffect(() => {
    ...
    const fetchAllRestockDates = async () => {
      ...
      ingredients.forEach((ingredient) => {
        initialLoadingState[ingredient.ingredient_id] = true;
      });
      ...
      try {
        ...
        if (data.success && Array.isArray(data.restockDetails)) {
          ...
          ingredients.forEach((ingredient) => {
            const restocks =
              groupedRestockDetails[ingredient.ingredient_id] || [];
            ...
            if (restocks.length > 0) {
              ...
              newLatestRestockDates[ingredient.ingredient_id] =
                restocks[0].restock_date;
            } else {
  ...
  const loadIngredients = async () => {
    try {
      ...
      fetchedIngredients.forEach((item) => {
        ...
        if (
          item.supplier_name &&
          !groupedIngredients[item.ingredient_id].suppliers.includes(
            item.supplier_name
          )
        ) {
          groupedIngredients[item.ingredient_id].suppliers.push(
            item.supplier_name
          );
        }
      });
  ...
  if (
    loading ||
    Object.values(loadingRestockDates).some((isLoading) => isLoading)
  ) {
  ...
  const calculateDaysBeforeExpired = (ingredient) => {
    const latestRestockDateString =
      latestRestockDates[ingredient.ingredient_id];
    ...
    <div className="ingredients-management">
      <div className="ingredients-header">
        ...
        <button
          ...
          style={{justifyContent:'flex-end', marginLeft:'75rem', backgroundColor:'#F59E0B' }}
        >
          + Add New Ingredient
        </button>
      </div>
      ...
      {filteredIngredients.length === 0 &&
        !searchTerm &&
        !loading &&
        !error && <div className="no-results">No ingredients available.</div>}
